...
The teacher-facing dashboard is the human-in-the-loop control surface of the
Kamerton booking agent. It runs on `localhost` only and shows, in real time,
what the agent is doing: active conversations with streamed agent text, the
live request card, the queue of `pending` requests (FR-DASH-01), and the
