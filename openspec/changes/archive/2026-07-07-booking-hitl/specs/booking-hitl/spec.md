...
> `booking-hitl` baseline spec already passed G2
> (`openspec/specs/booking-hitl/spec.md`) and no requirement text changes as
> part of this slice — this change only *implements* the already-accepted
> requirements below, unchanged. `MODIFIED` is used (with the full, unedited
...
> `openspec/specs/booking-hitl/spec.md` — the same convention the archived
> `slots`/`intake`/`dashboard` changes established.
