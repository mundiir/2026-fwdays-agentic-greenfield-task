# kb-learning Specification

## Purpose

Answer lead questions about the school strictly from `knowledge/school.md`,
log every lead question deterministically, and close the loop on unanswered
questions through the dashboard's Question-inbox panel — the one panel this
capability owns end-to-end (the `dashboard` capability owns only the
conversation and queue surfaces). The knowledge base grows only through human
...
- **No inbox deduplication or frequency counters** (FR-KB-05 is Future):
  duplicate questions each get their own inbox entry; the list is ordered by
  recency only, never by frequency.
- **No agent-drafted KB entries**: the agent never proposes or pre-fills
  answer text in the inbox; the admin writes the answer.
- **No automatic re-answering**: answering an inbox question notifies only
  the originating lead; other leads who asked the same thing are not
  retro-notified (they get the answer if they ask again).
- **No KB editing UI**: beyond the inbox append, `knowledge/school.md` is
